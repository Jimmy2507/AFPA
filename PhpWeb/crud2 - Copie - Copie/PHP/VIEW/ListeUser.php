...
if (count($listeUser)>0) {
    foreach ($listeUser as $u ) {
      echo '
            <div class="username">'.texte("nom").':'.$u->getUsername().'</div>
            <div class="Utilisateur">
            <a href="?page=FormUser&mode=detail&id='.$u->getIdUser().'"><button class="btn">'.texte("detail").'</button></a>
            <a href="?page=FormUser&mode=modifier&id='.$u->getIdUser().'"><button class="btn">'.texte("modifier").'</button></a>
            <a href="?page=FormUser&mode=supprimer&id='.$u->getIdUser().'"><button class="btn">'.texte("supprimer").'</button></a>  
            </div> 
    ';   
